...
APP_DIR="${DIST_DIR}/Screenie.app"
LEGACY_APP_DIR="${DIST_DIR}/SnapText.app"
BUILD_ARCH="$(/usr/bin/uname -m)"
ZIP_PATH="${DIST_DIR}/Screenie-${BUILD_ARCH}.zip"
...
(
    cd "${PROJECT_DIR}"
    /usr/bin/env swift build -c release --product Screenie
)

BIN_DIR="$(cd "${PROJECT_DIR}" && /usr/bin/env swift build -c release --show-bin-path)"
EXECUTABLE="${BIN_DIR}/Screenie"
...
/bin/rm -rf "${APP_DIR}" "${LEGACY_APP_DIR}"
/bin/rm -f \
    "${DIST_DIR}/Screenie.zip" \
    "${DIST_DIR}"/Screenie-*.zip \
    "${DIST_DIR}/SnapText.zip" \
    "${DIST_DIR}"/SnapText-*.zip
